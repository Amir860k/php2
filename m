...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['score'], $_POST['name_dars']) && is_numeric($_POST['score'])) {
        ...
        if ($score < 0 || $score > 20) {
            ...
            $stmtCheck = $pdo->prepare("SELECT id FROM studenn WHERE user_id=? AND name_dars=?");
            $stmtCheck->execute([$user_id, $name_dars]);
            $exists = $stmtCheck->fetch();

            if ($exists) {
                // آپدیت نمره و تاریخ/زمان
                $stmtUpdate = $pdo->prepare("UPDATE studenn SET score=?, date_time=? WHERE id=?");
                ...
                $stmtInsert = $pdo->prepare("INSERT INTO studenn (user_id, name_dars, score, date_time) VALUES (?,?,?,?)");
